...
function renderPapers() {
    ...
    container.innerHTML = filteredPapers.map(paper => {
        ...
        const summary = paper.summary_zh || paper.summary || '';
        const title = paper.title_zh || paper.title || '';

        const tldr = paper.tldr ? `<div class="paper-tldr">${paper.tldr}</div>` : '';
...
function openPaperDetail(paper) {
    ...
    const abstract = paper.summary_zh || paper.summary || '';
    if (abstract && !sections.length) sections.push(`<h3>Abstract</h3><p>${abstract}</p>`);

    detail.innerHTML = `
        <div class="paper-header">${sourceBadge}</div>
