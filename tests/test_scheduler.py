...
ORDER = ["arxiv", "crossref", "dblp", "s2", "author"]
EXPECTED_MINUTES = [0, 30, 60, 90, 120]  # index × 30min
...
s.stop()
print("[1] 默认启动不跑任务；02:00/02:30/03:00/03:30/04:00 错峰 ✓")
...
with patch.dict("os.environ", {"RUN_ON_START": "1"}):
    ...
    s3.start()
    time.sleep(0.2)
    assert sorted(ran[n_before:]) == ["arxiv", "author", "crossref", "dblp", "s2"]
    s3.stop()
