...
📖 **[完整功能指南](docs/GUIDE.md)** — 每个功能的详细说明、参数、操作入口
...
- **分时段动态限流 + 优先槽**：白天并发 6（实测标定）+ 指数退避（429/1302，抖动错峰）+ SDK 内部重试关闭；**凌晨低峰自动放宽**（默认 0-8 点并发 9/间隔减半，窗口与两档并发均可在 ⚙️ 面板调整，限值变化实时生效无需重启）；1 槽保留给交互式单发任务（简报/趋势/想法）——爬取风暴占满批量槽时点简报也不用排队
...
通用：侧边栏分组筛选（来源/会议/期刊/领域/推荐/收藏）+ 搜索 + 8 种排序（含⏱入库时间——增量纳入时最新进库论文置顶）、
...
| 板块 | 能改什么 | 生效方式 |
|:-----|:---------|:---------|
| 调度与限流 | 每日起始小时（0-23 任意时段）、LLM 凌晨放宽窗口/两档并发、任务错峰间隔、启动即跑、DBLP/S2 轮换天数、本地预筛开关 | 保存即重排时间表 |
...
顶栏功能按职责分为三个入口（互不掺杂），外加全局 **⚡ 任务中心**——常驻按钮显示运行中任务数（呼吸动画），点开面板看每个任务的**进度条 + n/N + 速度 + ETA**，完成后 ✓ 摘要留档；页面顶端 3px 渐变细进度条实时反映抓取批推进（论文正在流式入库时直观可见）：
...
- **任务互斥网关**：夜间计划任务同一时刻只跑一个，冲突自动顺延（手动多触发属有意并行，进度行标注"与 xx 并行抢LLM限额"）
- **流水线抓取**：边抓边分析（生产者-消费者 + 背压队列），爬取中断已抓部分仍完整处理；处理进度逐篇落盘，中断续跑只补未完成篇目
- **处理进度双通道**：近 3 分钟窗口即时速度（篇/分）+ ETA，日志与 🔄 菜单计数器同步刷新
